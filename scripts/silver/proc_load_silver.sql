    ...
    BEGIN TRY
		...
		SELECT
			...
			CASE
				...
				ELSE 'n/a'
			END AS cst_marital_status, -- Standradize marital status values to readable format
			CASE
		...
		SELECT
			prd_id,
			REPLACE(SUBSTRING(prd_key,1,5),'-','_') AS cat_id, -- Ectract category ID
		...
		SELECT
			...
			sls_quantity,
			CASE
				WHEN sls_price IS NULL OR sls_price <= 0 OR sls_price != ABS(sls_sales / sls_quantity) THEN sls_sales / sls_quantity
				ELSE sls_price
			END AS sls_price
